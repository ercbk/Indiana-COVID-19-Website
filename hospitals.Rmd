...
mort_staff_admiss_dat <- readr::read_rds(glue("{rprojroot::find_rstudio_root_file()}/Indiana-COVID-19-Tracker/data/hosp-msa-line.rds"))
...
fntltp <- JS("function(){
              ...
              '<i>' + this.points[2].series.name + '</i>' + ' : ' + this.points[2].y.toFixed(1);
}")
...
state_msa_line <- highchart() %>%
   ...
   hc_xAxis(categories = levels(mort_staff_admiss_dat$date),
   ...
   hc_add_yAxis(nid = 1L,
                ...
                showFirstLabel = FALSE, relative = 3) %>%
   hc_add_series(name = 'Hospital Mortality Rate', data = mort_staff_admiss_dat,
                 ...
                 dataLabels = list(
                    ...
                    filter = list(
                       ...
                       property = "index",
                       # 0-indexed
                       value = nrow(mort_staff_admiss_dat)-1
                    ),
   ...
   hc_add_yAxis(nid = 2L,
                labels = list(style = list(color = "white"),
                              formatter = yaxis_fmt),
                min = 0.14, max = 0.20,
                tickAmount = 4, showFirstLabel = FALSE, relative = 1) %>%
   hc_add_series(name = 'Hospitals with Staffing Shortages', data = mort_staff_admiss_dat,
   ...
   hc_add_yAxis(nid = 3L,
                ...
                showLastLabel = FALSE, relative = 2) %>% 
   hc_add_series(name = 'Hospital Admissions', data = mort_staff_admiss_dat,
...
The latest data values for each variable are labelled. Panning across the chart will display values for all variables on that day. Each line series can be selected or de-selected by clicking on names in the legend.
