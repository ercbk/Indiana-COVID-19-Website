...
age_bubble <- highchart() %>% 
   ...
   hc_add_series(name = 'Median Age', data = med_age_tbl, type = "bubble",
                 ...
                 zMax = 500, maxSize = "18%", marker = list(fillColor = '#99875c'),
...
high_cases <- highchart() %>%
   ...
   hc_xAxis(categories = levels(heat_dat$end_date),
            ...
            labels = list(step = 2)) %>% 
   hc_colorAxis(stops = color_stops(1000, inferno(1000, begin = 0.10)),
                # endontick=T only allows integers for min and max
                endOnTick = FALSE, max = 2500,
                # legend axis labels
                labels = list(
...
high_cap <- highchart() %>%
   ...
   hc_xAxis(categories = levels(heat_dat$end_date),
            ...
            labels = list(step = 2)) %>% 
   # 1000 adds more gradations (more continuous), begin = .10 starts palette in a lighter area
   hc_colorAxis(stops = color_stops(1000, inferno(1000, begin = 0.10)),
                # endontick=T only allows integers for min and max
                endOnTick = FALSE, max = 3.5,
                # legend axis labels
                labels = list(
                   style = list(color = "#ffffff")
                )) %>% 
